...
@Component
@Slf4j
public class SendEmailTaskHandler implements ScheduledTaskHandler {

    @Override
    public ScheduledTaskType supports() {
        return ScheduledTaskType.SEND_EMAIL;
    }

    @Override
    public TaskExecutionResult handle(ScheduledTask task) {
        log.info("Triggering email task {} with payload {}", task.getPublicId(), task.getPayload());
        // TODO integrate with Mail service when available
